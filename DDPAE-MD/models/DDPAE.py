...
class DDPAE(BaseModel):
  ...
  def __init__(self, opt):
    ...
    self.labels_mu = Variable(torch.zeros(1).cuda())
    self.labels_sigma = Variable(torch.ones(1).cuda())
  ...
  def setup_networks(self):
    ...
    # Backbone, Pose RNN
    pose_model = PoseRNN(self.n_components, self.n_frames_input, self.n_frames_output, self.n_channels,
  ...
  def sample_latent(self, input, input_latent_mu, input_latent_sigma, pred_latent_mu,
                    pred_latent_sigma, initial_pose_mu, initial_pose_sigma, labels_mu, labels_sigma, sample=True):
    '''
    ...
    # Note: get_transitions --> sample from latent
    ...
    # Note: RWH here? with pose after sampling.
    pose = self.constrain_pose(pose)

    # Get input objects
    input_pose = pose[:, :self.n_frames_input, :, :]

    #TODO: TRANSFORMA (AFFINE) L'INPUT DIRECTAMENT??
    ...
    md_labels = self.pyro_sample('missing', dist.Normal, labels_mu,
                                    labels_sigma, sample)

    latent.update({'pose': pose, 'content': z, 'missing': md_labels})
    return latent
  ...
  def sample_latent_prior(self, input):
    ...
    # Note: z_priors and z are 128*128
    ...
    # Note: input_beta_prior and z are 1280*3
    ...
    # Note: beta and z are N,20,2,3

    # Get pose
    pose = self.accumulate_pose(beta)
    # Note: pose and z are N,20,2,3
    ...
    K = batch_size * self.n_components * self.n_frames_input
    labels_mu = self.labels_mu.repeat(K, 1)
    labels_sigma = self.labels_sigma.repeat(K, 1)
    md_labels = self.pyro_sample('missing', dist.Normal, labels_mu,
                                    labels_sigma, sample=True)
    latent.update({'pose': pose, 'content': z, 'missing': md_labels})
    return latent
  ...
  def sample_content(self, content, sample):
    ...
    for i in range(self.total_components):
      z = content[:, :, i, :]
      # Note: Agafa la sequencia, la passa per un lstm 128->64 i l'ultim output per una fc 64->128
    ...
    # Note: sofplus is a linear version of ReLu
  ...
  def encode(self, input, sample=True):
    ...
    input_latent_mu, input_latent_sigma, pred_latent_mu, pred_latent_sigma,\
        initial_pose_mu, initial_pose_sigma, labels_mu, labels_sigma = self.pose_model(input)

    # Sample latent variables
    latent = self.sample_latent(input, input_latent_mu, input_latent_sigma, pred_latent_mu,
                                pred_latent_sigma, initial_pose_mu, initial_pose_sigma, labels_mu, labels_sigma, sample)
    return latent
  ...
  def decode(self, latent, batch_size):
    ...
    md_labels = latent['missing'].view(-1, self.n_frames_input, self.total_components,1,1,1)
    if self.n_frames_output > 0:
      md_labels_out = torch.ones(batch_size, self.n_frames_output, self.n_components, 1, 1, 1).cuda()
      md_labels = torch.cat([md_labels, md_labels_out],dim=1)  # TODO: check that the way components are viewed maches with how the labels are computed
    md_labels = self.sigmoid(md_labels)

    components = components #* md_labels
  ...
  def train(self, input, output):
    ...
    '''
    input = Variable(input.cuda(), requires_grad=False) #TODO: canviar a false
    ...
    for name, svi in self.svis.items():
      ...
      # NOTE: also my custom loss could be here and retain/create_graph?
