...
class PoseRNN(nn.Module):
  ...
  def __init__(self, n_components, n_frames_input, n_frames_output, n_channels, image_size,
    ...
    self.labels_rnn = nn.LSTM(hidden_size, hidden_size, num_layers=1, batch_first=True) #TODO: , bidirectional=True
    ...
    # self.labels_mu_layer_2 = nn.Linear(self.n_frames_input, self.n_frames_input)
    # self.labels_sigma_layer_2 = nn.Linear(self.n_frames_input, self.n_frames_input)
  ...
  def get_md_labels(self, repr):
    ...
    output, _ = self.labels_rnn(repr.view(-1, self.n_components, self.hidden_size))
    output = output.contiguous().view(-1, self.hidden_size)
    labels_mu = self.labels_mu_layer(output)
    labels_sigma = self.labels_sigma_layer(output)
    labels_sigma = F.softplus(labels_sigma)
    return labels_mu, labels_sigma
  ...
  def encode(self, input):
    ...
    for i in range(self.n_components):
      ...
      for j in range(n_frames_input):
        ...
        output, hidden = self.encode_rnn(rnn_input, hidden) #TODO: check what happens if we don't input the hidden from the previous
      ...
      frame_inputs = torch.cat(frame_inputs, dim=1) # for 1 component
    ...
    encoder_inputs = torch.stack(encoder_inputs, dim=2)
    ...
    labels_mu, labels_sigma = self.get_md_labels(encoder_outputs)

    return input_beta_mu, input_beta_sigma, initial_mu, initial_sigma,\
           encoder_outputs, hidden_states, labels_mu, labels_sigma
  ...
  def forward(self, input):
    ...
    input_beta_mu, input_beta_sigma, initial_mu, initial_sigma,\
        encoder_outputs, hidden_states, labels_mu, labels_sigma = self.encode(input)
    ...
    return input_beta_mu, input_beta_sigma, pred_beta_mu, pred_beta_sigma,\
           initial_mu, initial_sigma, labels_mu, labels_sigma
